...
- Todo envio de WhatsApp passa por `message_claims` (`services/messageClaim.ts`): a unique key `(user_id, kind, ref_key)` garante que só uma instância envia. **A trava só é liberada quando a não-entrega é certa** — `isDeliveryRuledOut` aceita 4xx (menos 408/429), `ECONNREFUSED`/`ENOTFOUND`/`EAI_AGAIN` e número inexistente. Timeout, `ECONNRESET` e 5xx mantêm a trava: o timeout de 10s do axios não cancela o envio do lado do WhatsApp, e liberar nesse caso é o que duplicava a mensagem. Consequência aceita: numa falha ambígua o envio do dia é perdido, não repetido.
- `POST /api/checklists/send-now` com `force` é a única duplicação permitida — apaga o poll do dia e libera a trava de propósito, porque é decisão explícita de quem clicou.
- Recorrência de checklist (`services/checklistRecurrence.ts`) usa o dia da semana em São Paulo. O container roda em UTC; usar `new Date().getDay()` fazia checklist das 21h BRT ser avaliado no dia seguinte.
...
- Foto de perfil do WhatsApp trafega como **data URI**, não como a URL do CDN. O `nginx.conf` serve `img-src 'self' data:`, então uma URL do `pps.whatsapp.net` é bloqueada pelo navegador e o card cai no ícone genérico — parece "não achou a foto", mas a API respondeu certo. `fetchWhatsAppProfile` baixa a imagem (teto de 2 MB, cache de 10 min por URL) e devolve embutida.
- `whatsapp_number` nem sempre é telefone: usuário vindo do webhook fica gravado com o LID (`51810291171433@lid`). `buildContactId` preserva o sufixo em vez de mandar tudo para `@c.us`, e o LID não vai para `/check-exists` (viraria "número não encontrado" falso — `numberExists` fica `null`).
- Quando o contato responde 200 com nome e foto vazios, `fetchWhatsAppProfile` repete a busca com a outra grafia do 9º dígito. A forma errada do número não dá erro, só devolve tudo vazio.
...
- Comparativo da carteira (`GET /api/assets/benchmark`): a rentabilidade desconta aportes — o retorno do dia é quanto a carteira de *ontem* (mesmas quantidades) vale nos preços de hoje (`services/benchmarkMath.ts`). CDI vem do Banco Central (série SGS 12, sem chave), IBOV da brapi (`^BVSP` com `range`), ambos em cache de 6h (`services/benchmarks.ts`). Fonte fora do ar vira `null` e o card mostra só o que tiver.
- Comandos por WhatsApp: o webhook assina também o evento `message` (`WEBHOOK_EVENTS` em `services/waha.ts`); registro antigo sem ele aparece como "não registrado" até re-registrar ou reiniciar. Só texto começando com `/` é comando (`/contas`, `/carteira`, `/hoje`, `/ajuda`) — o resto é ignorado em silêncio; `/algo` inexistente recebe a ajuda. Remetente é casado com `users.whatsapp_number` por telefone (com variante do 9º dígito) ou LID, com fallback no `/lids` do WAHA. A resposta vai para o `from` da mensagem e passa por `message_claims` (`command_reply`, chave = hash do id da mensagem) contra reentrega. Parte pura em `services/whatsappCommands.ts`, banco/envio em `services/whatsappCommandHandler.ts`. Mensagem comum não vai para o log do webhook.
- Resumo semanal (`sendWeeklySummary`, 8h do `summary_day_of_week`): contas do mês e dos próximos 7 dias, variação da carteira nos últimos 7 dias (descontando aportes) e checklists dos 7 dias fechados antes de hoje. Bloco sem dado some da mensagem.
